...
- The generated code targets Swift 6 concurrency checking: the entity query is a `static let`, and the resolver and handler protocols are `Sendable`, so a Swift 6.2 target accepts them. An app that registers a resolver or a handler must give it a `Sendable` implementation, and the example project still builds with `SWIFT_VERSION = 5.0`.
