...
IntentLane owns the output directory and writes exactly two files:
...
`apps/example-expo` is a minimal Expo app with two intents, English and French copy, and the plugin declared. It typechecks, and an iOS prebuild registers the generated Swift in the target:
...
- The generated Swift hardcodes the default locale. Per-locale Swift output is not implemented.
